...
def _create_fed(db, world_id="world1", name="TestFed", **kw) -> GameFederationDB:
    ...
    defaults = dict(
        ...
        weekly_revenue=0, weekly_expenses=0, momentum=50,
        ai_personality={
...
def _create_roster(db, fed, count=10):
    ...
    for i in range(count):
        w = _create_wrestler(db, name=f"Wrestler_{i}", popularity=random.randint(20, 90))
        db.add(ContractDB(
            world_id="world1", wrestler_id=w.id, federation_id=fed.id,
            status="active", salary_weekly=1000, start_date="2026-01-01",
        ))
        wrestlers.append(w)
    db.flush()
    return wrestlers
...
class TestVisionGeneration:
    def test_vision_created_with_push_tiers(self, db_session):
        ...
        champ = ChampionshipDB(
            ...
            current_holder_id=roster[0].id, is_active=True,
        )
    ...
    def test_vision_creates_push_records(self, db_session):
        ...
        push_records = db_session.query(WrestlerPushDB).filter(
    ...
    def test_workrate_style_prioritizes_in_ring(self, db_session):
        ...
        random.seed(42)
        _create_world(db_session)
        fed = _create_fed(db_session, ai_personality={
        ...
        stats = db_session.query(WrestlerStatsDB).filter(
            WrestlerStatsDB.wrestler_id == charisma_guy.id
        ).first()
        stats.charisma = 90
        stats.mic_skill = 90
        stats.technical = 30
        ...
        stats2 = db_session.query(WrestlerStatsDB).filter(
            WrestlerStatsDB.wrestler_id == tech_guy.id
        ).first()
        stats2.technical = 90
...
class TestPPVCalendar:
    ...
    def test_get_next_ppv(self, db_session):
        ...
        ppv = PPVEventDB(
            world_id="world1", federation_id=fed.id,
            name="Test PPV", theme="showcase",
...
class TestVisionAdaptation:
    ...
    def test_injury_strips_champion(self, db_session):
        ...
        fed.ai_personality = {"booking_style": "entertainment", "risk_tolerance": 20, "talent_priority": "mixed"}
    ...
    def test_departure_cleans_up_vision(self, db_session):
        ...
        push = db_session.query(WrestlerPushDB).filter(
    ...
    def test_cold_act_gets_demoted(self, db_session):
        ...
        if upper:
            ...
            adapt_vision_for_cold_act(db_session, vision, cold_wrestler_id, "2026-02-01")
...
class TestPushQueries:
    def test_get_push_tier(self, db_session):
        ...
        db_session.add(WrestlerPushDB(
            world_id="world1", federation_id=fed.id, wrestler_id=w.id,
            push_tier="main_event", direction="established", protected=True,
        ))
        db_session.flush()
    ...
    def test_get_tier_roster(self, db_session):
        ...
        for wid, tier in [(w1.id, "main_event"), (w2.id, "main_event"), (w3.id, "midcard")]:
            db_session.add(WrestlerPushDB(
                world_id="world1", federation_id=fed.id, wrestler_id=wid,
                push_tier=tier,
            ))
        db_session.flush()
...
class TestCareerGoals:
    def test_career_goals_generated(self, db_session):
        ...
        assert young_goals[0] in ["win_first_title", "prove_myself", "make_it_to_main_event"]
