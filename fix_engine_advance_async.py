...
content = content.replace("results = engine.run_ticks(n_ticks)", "results = await engine.run_ticks(n_ticks)")
content = content.replace("db.query(EngineRequestDB)", "await db.execute(select(EngineRequestDB))")
content = content.replace("db.query(NarrativeLogDB)", "await db.execute(select(NarrativeLogDB))")
