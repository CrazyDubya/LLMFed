...
from game_service.lifecycle_constants import (
    ...
    GOAL_FRUSTRATION_PER_WEEK, GOAL_FRUSTRATION_MORALE_THRESHOLD,
    GOAL_FRUSTRATION_MORALE_PENALTY, GOAL_FRUSTRATION_ALIGNMENT_THRESHOLD,
    GOAL_FRUSTRATION_ALIGNMENT_SHIFT,
    GLASS_CEILING_WEEKS, GLASS_CEILING_FRUSTRATION_BONUS, TITLE_GOAL_TYPES,
    LOW_SATISFACTION_THRESHOLD, LOW_SATISFACTION_MORALE_PENALTY,
    HIGH_SATISFACTION_THRESHOLD, HIGH_SATISFACTION_MORALE_BONUS,
    GOAL_PROVE_MYSELF_POP, GOAL_LEGACY_THRESHOLD, GOAL_TOP_DRAW_RATING,
    GOAL_EARN_RESPECT_POP, GOAL_EARN_RESPECT_MORALE, GOAL_HEADLINE_POP,
)
...
def evaluate_goals(db: Session, wrestler: GameWrestlerDB, game_date: str):
    ...
    for goal in active:
        if _check_goal_completed(db, wrestler, goal):
            ...
            wrestler.satisfaction = min(100, (wrestler.satisfaction or 50) + GOAL_COMPLETE_SATISFACTION_BONUS)
            ...
            if goal.frustration > GOAL_FRUSTRATION_MORALE_THRESHOLD:
                wrestler.morale = max(0, (wrestler.morale or 50) - GOAL_FRUSTRATION_MORALE_PENALTY)
                # Frustrated faces drift heel
                if wrestler.alignment == "face" and goal.frustration > GOAL_FRUSTRATION_ALIGNMENT_THRESHOLD:
                    wrestler.alignment_momentum = (wrestler.alignment_momentum or 0) + GOAL_FRUSTRATION_ALIGNMENT_SHIFT
    ...
    if push and (push.weeks_at_tier or 0) > GLASS_CEILING_WEEKS:
        ...
        for g in title_goals:
            g.frustration = min(100, (g.frustration or 0) + GLASS_CEILING_FRUSTRATION_BONUS)

    # Satisfaction influences morale
    sat = wrestler.satisfaction or 50
    if sat < LOW_SATISFACTION_THRESHOLD:
        wrestler.morale = max(0, (wrestler.morale or 50) - LOW_SATISFACTION_MORALE_PENALTY)
    elif sat > HIGH_SATISFACTION_THRESHOLD:
        wrestler.morale = min(100, (wrestler.morale or 50) + HIGH_SATISFACTION_MORALE_BONUS)
