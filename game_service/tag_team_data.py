...
def _classify_style(stats: WrestlerStatsDB) -> str:
    ...
    top = max(
        ("power", stats.power),
        ("highflyer", stats.aerial + stats.speed),
...
def generate_tag_team_name(
    ...
    """
    stats1 = (
        db.query(WrestlerStatsDB).filter(WrestlerStatsDB.wrestler_id == w1.id).first()
    )
    stats2 = (
        db.query(WrestlerStatsDB).filter(WrestlerStatsDB.wrestler_id == w2.id).first()
    )
    ...
    existing_names = {
        t.name for t in db.query(TagTeamDB).filter(TagTeamDB.world_id == world_id).all()
    }
