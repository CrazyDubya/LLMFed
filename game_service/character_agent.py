...
def _parse_tagged_response(text: str, tags: Dict[str, Any],
...
def character_decide(db: Session, wrestler_id: str,
    ...
    # Parse the response
    parsed = _parse_tagged_response(result, {"action": None, "reasoning": None}, fallback)
...
def character_social_media_post(db: Session, wrestler_id: str,
    ...
    system_prompt = build_character_system_prompt(db, wrestler_id)
    user_prompt = build_social_media_prompt(wrestler.name, platform,
...
def booker_decide_storyline(db: Session, federation_id: str,
    ...
    user_prompt = build_booker_storyline_prompt(fed_name, fed_style,
    ...
    parsed = _parse_tagged_response(
        ...
        {"type": {"feud", "alliance", "betrayal", "championship_chase"},
         "name": None, "description": None, "hook": None},
        fallback,
    )
...
def booker_decide_finish(db: Session, federation_id: str,
    ...
    user_prompt = build_booker_finish_prompt(wrestler1_name, wrestler2_name,
                                             storyline_context, is_ppv)
...
def generate_match_narrative(winner_name: str, loser_name: str,
    ...
    user_prompt = build_match_narrative_prompt(
        winner_name, loser_name, finish_type, finish_description,
        rating, key_spots, stipulation, is_title_match,
    )
