...
class AgentConfig(BaseModel):
    ...
    personality_prompt: Optional[str] = Field(
        default=None, description="Base personality prompt for agent"
    )
...
class PossibleAction(BaseModel):
    ...
    requires_target: bool = Field(
        default=False, description="Whether this action requires a target agent"
    )
...
class EventContext(BaseModel):
    ...
    event_type: str = Field(
        description="Type of event (e.g., 'MatchEvent', 'PromoEvent')"
    )
    ...
    available_actions: List[PossibleAction] = Field(
        description="Actions the agent can choose from"
    )
...
class AgentActionResponse(BaseModel):
    ...
    target_agent_id: Optional[str] = Field(
        default=None, description="Target agent ID if action requires one"
    )
    commentary: Optional[str] = Field(
        default=None, description="Agent's commentary on the action"
    )
    intensity: Optional[int] = Field(
        default=5, ge=1, le=10, description="Intensity level of the action"
    )
...
class RefereeCallResponse(BaseModel):
    ...
    call: str = Field(
        description="Referee's call (e.g., 'pinfall', 'submission', 'disqualification')"
    )
...
class CrowdReactionResponse(BaseModel):
    ...
    heat_adjustment: int = Field(
        default=0, description="How much this reaction affects crowd heat"
    )
...
class BackstageActionResponse(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Description of the action"
    )
...
class AgentCreateData(BaseModel):
    ...
    gimmick_description: str = Field(
        description="Description of the agent's wrestling character"
    )
    llm_config: Dict[str, Any] = Field(description="LLM configuration for this agent")
    federation_id: Optional[str] = Field(
        default=None, description="Federation this agent belongs to"
    )
    webhook_url: Optional[str] = Field(
        default=None, description="Webhook URL for event notifications"
    )
...
class AgentUpdateData(BaseModel):
    ...
    gimmick_description: Optional[str] = Field(
        default=None, description="Updated gimmick description"
    )
    llm_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Updated LLM configuration"
    )
    federation_id: Optional[str] = Field(
        default=None, description="Updated federation assignment"
    )
...
class FederationCreateData(BaseModel):
    ...
    max_agents: int = Field(
        default=20, description="Maximum agents allowed in federation"
    )
    is_active: bool = Field(
        default=True, description="Whether the federation is active"
    )
