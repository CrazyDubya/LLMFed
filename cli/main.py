...
def world_list():
    ...
    try:
        ...
        if not worlds:
            console.print(
                "[yellow]No worlds found. Use 'llmfed world create <name>'.[/yellow]"
            )
            return
...
def world_status(world_id: str = typer.Argument(..., help="World ID")):
    ...
    try:
        ...
        wrestler_count = (
            db.query(GameWrestlerDB).filter(GameWrestlerDB.world_id == world_id).count()
        )
...
def world_advance(
    ...
    try:
        ...
        with console.status(f"Advancing {days} day(s)..."):
            ...
            db.commit()
        console.print(
            f"[green]Advanced {days} day(s). New date: {result.get('new_game_date', '?')}[/green]"
        )
    except Exception as e:
        db.rollback()
        console.print(f"[red]Error: {e}[/red]")
...
def match_replay(match_id: str = typer.Argument(..., help="Match ID")):
    ...
    try:
        ...
        try:
            events = (
                db.query(MatchEventDB)
                .filter(MatchEventDB.match_id == match_id)
                .order_by(MatchEventDB.sequence)
                .all()
            )
            if events:
