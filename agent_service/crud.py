...
async def create_federation(
    db: AsyncSession, fed_data: FederationCreateData
) -> Federation:
    db_fed = FederationDB(
...
async def get_federation_by_id(
    db: AsyncSession, federation_id: str
) -> Optional[Federation]:
    result = await db.execute(
        select(FederationDB).filter(FederationDB.federation_id == federation_id)
    )
...
async def update_federation(
    db: AsyncSession, federation_id: str, update_data: FederationUpdateData
) -> Optional[Federation]:
    result = await db.execute(
        select(FederationDB).filter(FederationDB.federation_id == federation_id)
    )
...
async def get_agents_by_federation_id(
    db: AsyncSession, federation_id: str
) -> List[Agent]:
    result = await db.execute(
        select(AgentDB).filter(AgentDB.federation_id == federation_id)
    )
    return [Agent.model_validate(a) for a in result.scalars().all()]


async def update_agent(
    db: AsyncSession, agent_id: str, update_data: AgentUpdateData
) -> Optional[Agent]:
    result = await db.execute(select(AgentDB).filter(AgentDB.agent_id == agent_id))
    db_agent = result.scalars().first()
    if not db_agent:
