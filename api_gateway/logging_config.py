...
class RequestLogger:
    ...
    def log_request_end(
        self, request: Request, request_id: str, status_code: int, duration_ms: float
    ):
    ...
    def log_error(
        self, request: Request, request_id: str, error: Exception, duration_ms: float
    ):
...
class PerformanceMonitor:
    ...
    def get_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for endpoint, metrics in self.metrics.items():
            result[endpoint] = {
                **metrics,
                "avg_duration_ms": metrics["total_duration_ms"] / metrics["count"]
                if metrics["count"] > 0
                else 0,
